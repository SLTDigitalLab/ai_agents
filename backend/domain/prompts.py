...
- SCRIPT PURITY: For Sinhala, use Sinhala characters only; never emit Tamil, Devanagari/Hindi, or mixed-script words. For Tamil, use Tamil characters only; never emit Sinhala, Devanagari/Hindi, or mixed-script words. English technical identifiers covered below may remain in Latin characters.
- Before responding in Sinhala or Tamil, silently verify every non-Latin word uses only the requested language's writing system. Rewrite any contaminated word into natural vocabulary in the requested language.
