...
//  Main component 
const VoiceAgentPage = ({ audioOutput = null, renderPresentation = null }) => {
    ...
    const handleOpenAIMessage = useCallback(async (event) => {
        ...
        switch (msg.type) {
            ...
            case 'filler':
                    ...
                    if (!audioOutput && 'speechSynthesis' in window) {
    ...
    }, [configureOpenAISession, sendOpenAIEvent, user.name, user.username, audioOutput]);

    //  Gemini audio playback 
    const playGeminiAudioChunk = useCallback((base64Data) => {
    if (audioOutput) {
        audioOutput.sendPcm(base64Data);
        setIsSpeaking(true);
        return;
    }
...
const stopAllAudio = useCallback(() => {
    ...
    const startConversation = async () => {
        ...
        try {
            ...
            if (audioOutput) await audioOutput.prepare();
    ...
    const startOpenAISession = async () => {
        ...
        if (audioOutput) micStreamRef.current = stream;
        const pc     = new RTCPeerConnection();
        pcRef.current = pc;

        if (!audioOutput && !openaiAudioRef.current) {
        ...
        pc.ontrack = (e) => {
            if (audioOutput) {
                audioOutput.connectStream(e.streams[0] || new MediaStream([e.track]));
            } else {
                openaiAudioRef.current.srcObject = e.streams[0];
            }
        };
        ...
        dc.onerror   = () => { audioOutput?.reset(); setErrorMessage('Connection error. Please try again.'); setPhase(PHASE.ERROR); };
    ...
    const startGeminiSession = async () => {
            ...
            try {
                ...
                switch (msg.type) {
                    ...
                    case 'transcript':
                        ...
                        break;
                    case 'turn_complete':
                        if (audioOutput) setIsSpeaking(false);
                        setTranscript(prev => {
                            const last = prev[prev.length - 1];
                            if (last?.role === 'assistant' && last?.partial)
                                return [...prev.slice(0, -1), { role: 'assistant', text: last.text }];
                    ...
                    case 'fallback_to_openai':
                        console.info('Gemini unavailable — switching to OpenAI');
                        cleanupGemini();
                        try {
                            if (audioOutput) await audioOutput.prepare();
        ...
        ws.onclose = () => {
            if (geminiWsRef.current === ws) audioOutput?.reset();
            if (phase === PHASE.CONNECTED) {
                setStatusText('Call ended — click Start to reconnect');
                setPhase(PHASE.IDLE);
            }
        };
        ws.onerror = () => { audioOutput?.reset(); setErrorMessage('Connection to voice backend failed'); setPhase(PHASE.ERROR); };
    };
    ...
    // Optional route-specific presentation; conversation logic remains above.
    if (renderPresentation) {
        return renderPresentation({
            ...
            theme, toggleTheme,
            backToChat: () => { endConversation(); navigate('/workmateai'); },
        });
    }
