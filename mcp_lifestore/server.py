...
import httpx
from services.lifestore_availability import check_live_availability, valid_product_url
from services.lifestore_identity import (
    CLARIFICATION, clean_identity_query, identity_tokens, select_identity_product,
...
# v2 excludes images cached by the old page-wide heuristic.
IMAGE_CACHE_PATH = Path(__file__).parent / "cache" / "lifestore_image_cache_v2.json"
...
def _extract_image_candidates_from_html(product_url: str, html: str) -> list[str]:
    """
    Extract images from Product structured data bound to the requested URL.
    """
    ...
    # Only Product structured data explicitly bound to this URL can establish
    # image ownership. Page-wide img/OG tags can describe recommendations.
    def visit(value):
        if isinstance(value, list):
            for item in value:
                visit(item)
            ...
            owner = value.get("url") or value.get("@id")
            if ("Product" in types and isinstance(owner, str)
                    and _normalize_image_lookup_key(urljoin(product_url, owner))
                    == _normalize_image_lookup_key(product_url)):
...
def _resolve_product_image(product_url: str, current_image_url: str = "", allow_page_fetch: bool = True) -> dict[str, Any]:
    """
    Return a frontend-safe product image.

    Resolution priority:
    1. Existing graph/local image, but only if it is a real product image.
    2. Verified product-image lookup keyed by the exact product URL.
    3. Versioned cache populated only by product-bound sources.
    ...
    if not allow_page_fetch:
        return {"image_url": "", "image_available": False, "image_source": "deferred"}
    ...
    try:
        ...
        # A redirect to another product must not populate this product's cache.
        if _normalize_image_lookup_key(str(response.url)) != _normalize_image_lookup_key(product_url):
            return {"image_url": "", "image_available": False, "image_source": "redirect_rejected"}
...
def _same_image_product(product: dict[str, Any], candidate: dict[str, Any]) -> bool:
    """Exact identity only; contradictory identifiers veto weaker name matches."""
    matched = False
    for field in ("url", "product_id", "sku", "model", "model_number"):
        left, right = product.get(field), candidate.get(field)
        if not left or not right:
            continue
        normalize = _normalize_image_lookup_key if field == "url" else identity_tokens
        if normalize(left) != normalize(right):
            return False
        matched = True
    name = identity_tokens(product.get("name"))
    return matched or bool(name and name == identity_tokens(candidate.get("name")))
...
def _merge_graph_and_vector_results(
    ...
    for row in graph_rows:
        ...
        # Metadata belongs to its document's exact identity, never its score or
        # a product mentioned in its prose. Preserve the record's own image.
        vector_image = False
        if _bad_image_url(product["image_url"]):
            for candidate in _vector_identity_candidates(vector_docs):
                candidate_image = candidate.get("image_url")
                if (_same_image_product(row, candidate)
                        and not _bad_image_url(candidate_image)):
                    product["image_url"] = candidate_image
                    vector_image = True
                    break
        ...
        if vector_image:
            image_result.update(image_source="exact_vector_metadata",
                                image_note="Image metadata matched the resolved product identity.")
        product.update(image_result)
        ...
        if _bad_image_url(product.get("image_url")):
            product["image_url"] = ""
            product["image_available"] = False
            if product.get("image_source") != "deferred":
                product["image_source"] = "not_available"
            product["image_note"] = "No valid product image found."
    ...
    for doc in vector_docs:
        ...
        candidates = _vector_identity_candidates([doc])
        matched = False

        for key, product in products_by_key.items():
            if any(_same_image_product(product, candidate) for candidate in candidates):
...
def lifestore_hybrid_product_search(
    ...
    # Price requests need a broad relevance-filtered pool before the backend
    # performs numeric sorting. This bounded pool is not a full-catalog claim.
    price_candidates = price_candidates is True
    limit = bounded_int(limit, default=5, min_value=1, max_value=100 if price_candidates else 8)
    ...
    if not price_candidates and search_mode in {"single_product", "availability", "purchase", "exact"}:
        limit = min(limit, 1)
    ...
    vector_docs = [] if price_candidates else _qdrant_search_documents(query=retrieval_query, limit=max(limit * 2, 8))
...
def _graph_identity_rows(query: str) -> list[dict[str, Any]]:
    """Read identity candidates without semantic ranking, stock filtering or early LIMIT."""
    driver = neo4j_driver()
    if driver is None:
        return []
    terms = identity_tokens(clean_identity_query(query))
    if not terms:
        driver.close()
        return []
    cypher = """
    ...
    WHERE any(term IN $terms WHERE
        any(value IN [p.name, p.product_id, p.sku, p.model, p.model_number]
            WHERE toLower(coalesce(toString(value), '')) CONTAINS term))
        OR p.url = $query
        OR any(value IN [p.product_id, p.sku, p.model, p.model_number] WHERE
            reduce(value_l = toLower(coalesce(toString(value), '')), separator IN ['-', '_', ' ', '.'] |
                   replace(value_l, separator, '')) = $compact)
    OPTIONAL MATCH (p)-[:MADE_BY]->(b:Brand)
    OPTIONAL MATCH (p)-[:BELONGS_TO]->(c:Category)
    RETURN p {.*, brand: coalesce(p.brand, b.name), category: coalesce(p.category, c.name)} AS product
    """
    try:
        with driver.session() as session:
            return [make_json_safe(dict(row["product"])) for row in session.run(
                cypher, terms=terms, query=query, compact="".join(terms))]
    except Exception:
        return []
    finally:
        driver.close()
...
PRECISE_CANDIDATE_LIMIT = 32


def _vector_identity_candidates(docs):
    """Use explicit product metadata/headings, never prose or similarity scores."""
    candidates = []
    for doc in docs[:PRECISE_CANDIDATE_LIMIT]:
        metadata = doc.get("metadata") or {}
        url = next((safe_text(value) for value in (
            metadata.get("url"), metadata.get("link"), metadata.get("source_url"),
            doc.get("link"), doc.get("source")) if valid_product_url(value)), "")
        if not url:
            continue
        name = safe_text(metadata.get("product_name") or metadata.get("name")
                         or metadata.get("Header 1") or metadata.get("title") or doc.get("title"))
        # Ingestion stores heading breadcrumbs as 'Product name > Overview'.
        name = name.split(" > ")[0].removesuffix(" | LifeStore").strip()
        if not name:
            continue
        candidate = {key: metadata[key] for key in
                     ("product_id", "sku", "model", "model_number", "brand", "image_url")
                     if metadata.get(key)}
        candidate.update(name=name, url=url, stock=None, stock_status="unknown")
...
def lifestore_precise_product_lookup(
    ...
    # Include local identities even if graph search returned similar products.
    # Graph facts take precedence when both sources contain the same product.
    rows = _graph_identity_rows(product_query)
    local_rows = []
    for raw in load_products():
        p = normalize_product(raw)
        local_rows.append({**raw, **p, "stock_status": p.get("availability")})
    # Broader search generates candidates only. Its score/order cannot select identity.
    broad_rows = _graph_product_search_rows(product_query, limit=PRECISE_CANDIDATE_LIMIT)
    vector_docs = _qdrant_search_documents(product_query, limit=PRECISE_CANDIDATE_LIMIT)
    candidates = dedupe_identity_candidates(
        rows + local_rows + broad_rows[:PRECISE_CANDIDATE_LIMIT] + _vector_identity_candidates(vector_docs))
    product = select_identity_product(product_query, candidates)
    if product is None:
        return {"status": "not_found", "products": [], "message": CLARIFICATION,
                "answer": CLARIFICATION, "retrieval": {"resolution": "identity_not_established"}}
    # Semantic evidence can describe a resolved identity, never select it.
    products = _merge_graph_and_vector_results(
        graph_rows=[product], vector_docs=vector_docs if include_vector_evidence else [], query=product_query,
        limit=1, allow_page_fetch=False,
    )
    return {"status": "success", "products": products,
            "answer": _build_answer_summary(product_query, products),
            "retrieval": {"resolution": "identity_match", "returned_products": len(products)}}
...
def lifestore_availability_lookup(
    ...
    product = {**products[0], **check_live_availability(products[0].get("url"))}
    actual = product["stock_status"]
    matches = product_matches_availability(
        {"availability": actual, "stock": product.get("stock")},
        requested_availability,
    ) if product["availability_verified"] else None

    return {
        "status": "success",
        "availability_verified": product["availability_verified"],
        "source": "live_website",
        "product_query": product_query,
        "requested_availability": requested_availability,
